...
import com.future.config.MyUserDetailsService;
 ...
 * 功能概要  :确保在一次请求只通过一次filter，而不需要重复执行
...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ResultForm contentResultForm = new ContentResultForm<>(false,null,"");
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (redisUtil.isBlackList(authToken)) {
                ...
                contentResultForm.setMessage("TOKEN_IS_BLACKLIST");
                response.getWriter().write(JSONUtil.toJsonStr(contentResultForm));
                return;
            }
            ...
            if (redisUtil.hasKey(authToken)) {//判断redis是否有保存
                ...
                if (JwtTokenUtil.isExpiration(expirationTime)) {
                    ...
                    if (DateUtil.compareDate(currentTime, tokenValidTime)) {
                        ...
                        contentResultForm.setMessage("LOGIN_IS_OVERDUE");
                        response.getWriter().write(JSONUtil.toJsonStr(contentResultForm));
                        return;
