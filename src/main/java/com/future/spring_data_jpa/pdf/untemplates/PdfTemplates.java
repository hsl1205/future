...
public class PdfTemplates {
    ...
    public void  demo() {
        ...
        String pdfName = UUID.randomUUID().toString().substring(0, 8);
        File file = new File("D:\\Pdf\\"+pdfName+".pdf");
        try {
            file.createNewFile();
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(file));
            //水印
            Watermark watermark = new Watermark("vivianus");
...
//            writer.setPageEvent(headerFooter);
            ...
            document.open();
            //标题
            document.addTitle("测试生成pdf文档");
            //作者
            document.addAuthor("Author@zhangs");
            //主题
            document.addSubject("Subject@iText pdf sample");
            //关键字
            document.addKeywords("Keywords@iTextpdf");
            //创建者
            document.addCreator("creator@hsl");
